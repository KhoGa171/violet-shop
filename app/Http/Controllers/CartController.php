...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            $already_cart = Cart::where('session_id', $id_user)->where('order_id', null)->where('product_id', $product->id)->first();
        }
        if ($already_cart) {
            // dd($already_cart);
            ...
            if ($already_cart->product->stock < $already_cart->quantity || $already_cart->product->stock <= 0) return back()->with('Lỗi', 'Mặt hàng này đã hết!.');
            $already_cart->save();
        } else {
            ...
            if(auth()->user()->id) 
                $cart->user_id = auth()->user()->id;
                 ...
                 $cart->user_id = null;
            $cart->session_id = $id_user;
            $cart->product_id = $product->id;
            $cart->price = ($product->price - ($product->price * $product->discount) / 100);
            $cart->quantity = 1;
            $cart->size = $request->size;
            $cart->amount = $cart->price * $cart->quantity;
            if ($cart->product->stock < $cart->quantity || $cart->product->stock <= 0) return back()->with('error', 'Mặt hàng này đã hết!.');
            $cart->save();
            if(Auth::check())
                $wishlist = Wishlist::where('user_id', auth()->user()->id)->where('cart_id', null)->update(['cart_id' => $cart->id]);
            else   $wishlist = Wishlist::where('session_id', $id_user)->where('cart_id', null)->update(['cart_id' => $cart->id]);  
        }
    ...
    public function singleAddToCart(Request $request)
    {
        ...
        if ($already_cart) {
            ...
            if ($already_cart->product->stock < $already_cart->quantity || $already_cart->product->stock <= 0) return back()->with('error', 'Mặt hàng này đã hết!.');
            ...
            if (Auth::check()) {
                $cart->user_id = auth()->user()->id;
            } else $cart->user_id = null;
            $cart->session_id = $id_user;
            $cart->product_id = $product->id;
            $cart->price = ($product->price - ($product->price * $product->discount) / 100);
            $cart->size = $request->size;
            $cart->quantity = $request->quant[1];
            $cart->amount = $cart->price * $cart->quantity; //($product->price * $request->quant[1]);
            if ($cart->product->stock < $cart->quantity || $cart->product->stock <= 0) return back()->with('error', 'Mặt hàng này đã hết!.');
            // return $cart;
            ...
            if(Auth::check()){
                ...
                $wishlist = Wishlist::where('session_id', $id_user)->where('cart_id', null)->update(['cart_id' => $cart->id]);
            }
        }
        $request->session()->put('session_id', $id_user);
        request()->session()->flash('success', 'Sản phẩm đã được thêm vào giỏ hàng.');
